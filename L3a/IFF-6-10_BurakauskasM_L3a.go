...
/*
func FindIndex(availableCars []Order, year int) int {
	for i := 0; i < len(availableCars); i++ {
		if availableCars[i].year == year {
			return i
		}
	}
	return -1
}
func AddCar(availableCars []Order, year int) []Order {
	var index = FindIndex(availableCars, year)
	
	if index == -1 {
		var index = len(availableCars)
		for i := 0; i < len(availableCars); i++ {
			if availableCars[i].year > year {
				index = i
				break
		...
		availableCars = append(availableCars, Order{0, 0, -1})
		
		copy(availableCars[index+1:], availableCars[index:])
...
func RemoveCar(availableCars []Order, year int) ([]Order, bool) {
	var index = FindIndex(availableCars, year)
	if index == -1 {
		return availableCars, false
	} else {
		if availableCars[index].count > 1 {
			availableCars[index].count--
		} else {
			copy(availableCars[index:], availableCars[index+1:])
			availableCars[len(availableCars)-1] = Order{0, 0, -1}
			availableCars = availableCars[:len(availableCars)-1]
		}
	}
	
	return availableCars, true
}
*/
...
func DataManager(countOutputChan chan<- int, orderOutputChansChan chan []chan []Order, carOutputChansChan chan []chan []Car, resultInputChan <-chan []Order) {
	...
	fmt.Println("data manager finished")
}
...
func Controller(consumersCount int, producersCount int, orderInputChan <-chan Order, carInputChan <-chan Car, responseOutputChanToConsumers []chan bool, responseOutputChanToProducers []chan bool, resultOutputChan chan<- []Order) {
	...
	// fmt.Println(consumersCount)
	// fmt.Println(producersCount)
	...
	for consumersCount > 0 || producersCount > 0 {
		...
		case message := <-carInputChan:
			...
			if message.year == -1 {
				...
				// availableCars = AddCar(availableCars, message.year)
		...
		case message := <-orderInputChan:
			...
			if message.year == -1 {
				...
				// availableCars, wasRemoved = RemoveCar(availableCars, message.year)
				responseOutputChanToConsumers[senderIndex] <- wasRemoved
				responseOutputChanToConsumers[senderIndex] <- producersCount > 0
			}
		}
		// fmt.Println(strconv.Itoa(consumersCount) + " " + strconv.Itoa(producersCount))
	}
